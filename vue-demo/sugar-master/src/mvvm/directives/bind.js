...
function updateClass (element, classValue, remove) {
	if (isString(classValue)) {
		...
		each(classValue, function (add, cls) {//add是value,如果存在，就不remove.不存在，
...
/**
 * v-bind 指令解析模块  这里的实现就是寄生组合式继承。
 */
export function VBind () {
	Parser.apply(this, arguments);//继承Parser的方法。
}

let vbind = linkParser(VBind);//继承Parser的原型方法。VBind.prototype = Object.create(Parser.prototype)

/**
 * 解析 v-bind 指令
 */
vbind.parse = function () {
	this.desc.deep = true;//desc属性是什么
	this.bind();
}
...
vbind.processClass = function (newClass, oldClass) {
	...
	if (oldClass) {
		...
		updateClass(el, before, true);//这里是将之前的属性多余的或者是需要重新修改的全部删除
		updateClass(el, after);//这是更新成最新的。
	} else {
