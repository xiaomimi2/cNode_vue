...
function makeVforGuid () {
	return Math.random().toString(36).substr(2);//"ipr6h9wvkz9t8kgu65no03sor"这个的结果很神奇，解释见网址https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Number/toString
}
...
vfor.parse = function () {
	...
	if (parent.nodeType !== 1) {//不是元素，说明已经在根节点上面了
	...
	let alias = match[1];//子项
	let iterator = match[2];//父项
	...
	this.bodyDirs = el.__dirs__;//这个好像是vfor的唯一标示符
	this.tpl = el.cloneNode(true);//复制所有的后代节点。
	this.hooks = getHooks(this.vm, el);
	this.isOption = el.tagName === 'OPTION' && parent.tagName === 'SELECT';

	desc.expression = iterator;//这是匹配的for of 后面的选项。
	this.bind();
	this.updateModel();
}
...
vfor.updateModel = function () {
	if (this.isOption) {
		let selectModel = this.$parent.__vmodel__;//select元素上有vmodel属性
		if (selectModel) {
...
vfor.buildList = function (list, startIndex) {
	...
	let iterator = this.directive.watcher.value;//为什么directive上有watcher
	...
	each(list, function (item, i) {
		...
		// 挂载别名
		def(scope, '__alias__', alias);//这个是定义属性defineProperty
		// 挂载迭代器
		...
		// 片段挂载别名
		def(frag, vforAlias, alias);//vforAlias = '__vfor__';
		// 挂载唯一 id
